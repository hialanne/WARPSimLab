...
from src.warpsimlab.reports.report_data import SpendingComparisonReportData
...
def _build_case_result(spending_percentage, deterministic_pipeline_result, historical_pipeline_result):
    ...
    deterministic_expenses = np.asarray(deterministic_core["expense_amt"], dtype=float)
    deterministic_taxes = np.asarray(deterministic_core["taxes"], dtype=float)
    deterministic_cash_flow_shortfall = np.asarray(deterministic_core["cash_flow_shortfall"], dtype=float)
    deterministic_uncovered_expense = np.asarray(deterministic_core["uncovered_expense"], dtype=float)

    deterministic_ending_portfolio = float(deterministic_total_assets[0, -1])
    deterministic_first_year_expenses = float(deterministic_expenses[0, 1]) if deterministic_expenses.shape[1] > 1 else 0.0
    deterministic_lifetime_expenses = float(np.sum(deterministic_expenses[0]))
    deterministic_lifetime_taxes = float(np.sum(deterministic_taxes[0]))
    deterministic_lifetime_cash_flow_shortfall = float(np.sum(deterministic_cash_flow_shortfall[0]))
    deterministic_lifetime_uncovered_expense = float(np.sum(deterministic_uncovered_expense[0]))
    ...
    return {
        ...
        "scenario_expense_multiplier": float(spending_percentage) / 100.0,
        "is_current_spending": float(spending_percentage) == 100.0,

        # Deterministic quantities
        "deterministic_ending_portfolio": deterministic_ending_portfolio,
        "deterministic_first_year_expenses": deterministic_first_year_expenses,
        "deterministic_lifetime_expenses": deterministic_lifetime_expenses,
        "deterministic_lifetime_taxes": deterministic_lifetime_taxes,
        "deterministic_lifetime_cash_flow_shortfall": deterministic_lifetime_cash_flow_shortfall,
        "deterministic_lifetime_uncovered_expense": deterministic_lifetime_uncovered_expense,
...
def run_sim_spending_comparison_report(
    ...
    original_expense_multiplier = getattr(sim_config, "scenario_expense_multiplier", 1.0)
    ...
    try:
        ...
        # Real estate is excluded to match the existing portfolio risk analysis.
        sim_config.include_realestate = False

        # The report calculates depletion directly from the Historical Window run.
        sim_config.show_simulated_shortfall_rate = False
        sim_config.calculate_simulated_shortfall_rate = False

        # Prevent run_pipeline() from launching unrelated overlay simulations for every comparison case.
        ...
        for spending_percentage in spending_percentages:
            sim_config.scenario_expense_multiplier = float(spending_percentage) / 100.0
            ...
            # Use all Historical Windows for current spending and a subset for comparison cases.
            if abs(float(spending_percentage) - 100.0) < 1e-9:
        ...
        sim_config.scenario_expense_multiplier = original_expense_multiplier
    ...
    return generate_spending_comparison_report(report_data)
