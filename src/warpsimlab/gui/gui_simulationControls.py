...
class SimulationControlsEditFrame(ScalableFrameMixin, ttk.Frame):
    ...
    def __init__(self, parent, control_vars, title="Simulation Controls"):
        ...
        self._panel_column_minsize = 300
        self._panel_gap = 30

        self._left_panel = None
        self._right_right_col = None
        ...
        ttk.Label(
            header_frame, text="Simulation > Controls", font=self._header_font
        ).pack(side="left")
        ...
        panel_column_minsize = max(1, round(self._panel_column_minsize * scale))
        panel_gap = max(1, round(self._panel_gap * scale))

        self._left_panel = ttk.Frame(self)
        self._left_panel.grid(
            row=1, column=0, columnspan=2, sticky="nw", padx=(0, panel_gap)
        )
        ...
        left_panel.columnconfigure(0, weight=0, minsize=panel_column_minsize)
        left_panel.columnconfigure(1, weight=0, minsize=panel_column_minsize)
        ...
        self._right_right_col = ttk.Frame(right_panel)
        self._right_right_col.grid(
            row=0, column=1, sticky="nw", padx=(panel_gap, 0)
        )
        right_right_col = self._right_right_col
        ...
        def add_left_check(label, key, tooltip_text=None):
            ...
            cb = ttk.Checkbutton(
                left_panel, text=label, variable=var,
                style="SimulationControls.TCheckbutton"
            )
            ...
            Tooltip(cb, tooltip_text, font=self._tooltip_font)
            left_col0_row += 1
        ...
        ttk.Label(
            left_panel, text="Overlays", font=self._section_font
        ).grid(row=left_col1_row, column=1, sticky="w", pady=(0, 5))
        ...
        annotations_section.grid(
            row=annotations_row, column=0, columnspan=2, sticky="nw", pady=(8, 2)
        )
        ...
        ttk.Label(
            annotations_header, text="Custom Annotations", font=self._section_font
        ).pack(side="left")
        ...
        ttk.Label(
            right_left_col, text="Real/Nominal Mode", font=self._body_font
        ).grid(row=right_col0_row, column=0, sticky="w", pady=(0, 2))
        ...
        real_rb = ttk.Radiobutton(
            ...
            value="real",
            style="SimulationControls.TRadiobutton"
        )
        real_rb.grid(row=0, column=0, sticky="w", pady=1)

        Tooltip(
            real_rb, "Show inflation-adjusted values.", font=self._tooltip_font
        )
        ...
        Tooltip(
            raw_rb, "Show non-inflation-adjusted nominal values.",
            font=self._tooltip_font
        )
        ...
        # Plot Style
        ttk.Label(
            right_left_col, text="Plot Style", font=self._body_font
        ).grid(row=right_col0_row, column=0, sticky="w", pady=(14, 2))
        ...
        subplot_mode_value_to_label = {
            value: label for label, value in subplot_mode_label_to_value.items()
        }
        ...
        subplot_mode_var = tk.StringVar(
            value=subplot_mode_value_to_label[results_mode]
        )
        ...
        # -------------------------------------------------
        # RIGHT PANEL: Real Estate visibility tied to Plot Style
        # -------------------------------------------------
        include_realestate_section = ttk.Frame(right_right_col)
        include_realestate_section.grid(row=0, column=0, sticky="nw")
        ...
        Tooltip(
            ...
            "Include real estate in simulation results. Hidden when Plot Style is Risk Analysis.",
            font=self._tooltip_font
        )
        ...
        # -------------------------------------------------
        # RIGHT PANEL: Risk Analysis options
        # -------------------------------------------------
        monte_carlo_section = ttk.Frame(right_right_col)
        monte_carlo_section.grid(row=0, column=0, sticky="nw")

        ttk.Label(
            monte_carlo_section, text="Risk Analysis Mode", font=self._body_font
        ...
        ttk.Label(
            monte_carlo_section, text="Risk Analysis Plot Style", font=self._body_font
        ...
        Tooltip(
            mc_plot_fill_rb,
            "Display Monte Carlo results using a filled band.",
            font=self._tooltip_font
        )
        ...
        Tooltip(
            mc_plot_line_rb,
            "Display Monte Carlo results using summary lines.",
            font=self._tooltip_font
        )
        ...
        Tooltip(
            mc_plot_all_lines_rb,
            "Display all Monte Carlo paths as lines.",
            font=self._tooltip_font
        )
        ...
        monte_carlo_plot_style_var.trace_add(
            "write", on_monte_carlo_plot_style_changed
        )
        ...
        correlated_returns_var.trace_add(
            "write", on_correlated_returns_changed
        )

        historical_note = ttk.Label(
            monte_carlo_section,
            text=(
                "Historical Windows uses annual returns\n"
        ...
        def update_monte_carlo_visibility(*_):
            is_monte_carlo = (
                subplot_mode_label_to_value[subplot_mode_var.get()] == "risk_analysis"
            )
    ...
    def _apply_scaled_styles(self):
        ...
        style.configure("SimulationControls.TCheckbutton", font=self._body_font)
        style.configure("SimulationControls.TRadiobutton", font=self._body_font)
        ...
        if self._left_panel is not None:
            self._left_panel.columnconfigure(0, minsize=panel_column_minsize)
            self._left_panel.columnconfigure(1, minsize=panel_column_minsize)
            self._left_panel.grid_configure(padx=(0, panel_gap))

        if self._right_right_col is not None:
            self._right_right_col.grid_configure(padx=(panel_gap, 0))
