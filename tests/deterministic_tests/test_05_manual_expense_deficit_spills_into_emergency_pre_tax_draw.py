...
def test_manual_expense_deficit_spills_into_emergency_pre_tax_draw_nominal():
    husband = make_person(age=40, retire_age=100, income=20_000.0)
    expenses = FlatExpenses(90_000.0)
    husband_portfolio = make_portfolio(equity_pre=100_000.0, equity_post=30_000.0)
    cfg = make_sim(years=1, inflation_rate=0.0, plot_mode="raw")

    results = run_sim(cfg, husband, expenses, husband_portfolio)

    assert row(results, "gross_income") == pytest.approx([0.0, 60_000.0])
    assert row(results, "net_income") == pytest.approx([0.0, 20_000.0])
    assert row(results, "net_profit") == pytest.approx([0.0, -70_000.0])

    assert row(results, "emergency_pre_tax_used") == pytest.approx([0.0, 40_000.0])
    assert row(results, "post_tax_assets") == pytest.approx([30_000.0, 0.0])
    assert row(results, "pre_tax_assets") == pytest.approx([100_000.0, 60_000.0])
    assert row(results, "total_assets") == pytest.approx([130_000.0, 60_000.0])


def test_manual_expense_deficit_spills_into_emergency_pre_tax_draw_real_mode_deflates_correctly():
    husband = make_person(age=40, retire_age=100, income=20_000.0)
    expenses = FlatExpenses(90_000.0)
    husband_portfolio = make_portfolio(equity_pre=100_000.0, equity_post=30_000.0)
    infl = 0.03
    ...
    for key in ["gross_income", "expense_amt", "emergency_pre_tax_used", "post_tax_assets", "pre_tax_assets", "total_assets"]:
