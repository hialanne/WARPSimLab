...
def test_manual_expense_deficit_uses_post_tax_assets_first_nominal():
    ...
    assert row(results, "gross_income") == pytest.approx([0.0, 20_000.0])
    assert row(results, "net_income") == pytest.approx([0.0, 20_000.0])
    assert row(results, "expense_amt") == pytest.approx([0.0, 50_000.0])
    assert row(results, "emergency_pre_tax_used") == pytest.approx([0.0, 0.0])

    assert row(results, "post_tax_assets") == pytest.approx([100_000.0, 70_000.0])
    assert row(results, "pre_tax_assets") == pytest.approx([80_000.0, 80_000.0])
    assert row(results, "total_assets") == pytest.approx([180_000.0, 150_000.0])


def test_manual_expense_deficit_uses_post_tax_assets_first_real_mode_deflates_correctly():
    husband = make_person(age=40, retire_age=100, income=20_000.0)
    expenses = FlatExpenses(50_000.0)
    husband_portfolio = make_portfolio(equity_pre=80_000.0, equity_post=100_000.0)
    infl = 0.03
    ...
    for key in ["gross_income", "expense_amt", "post_tax_assets", "pre_tax_assets", "total_assets"]:
