...
def test_percent_of_portfolio_retirement_withdrawal_nominal():
    husband = make_person(age=70, retire_age=65)
    husband_portfolio = make_portfolio(equity_post=200_000.0)
    cfg = make_sim(years=3, inflation_rate=0.0, inflation_mode="nominal")

    results = run_sim(cfg, husband, husband_portfolio)

    assert row_class(results, "withdrawal") == pytest.approx([0.0, 20_000.0, 20_000.0, 20_000.0])
    assert row(results, "post_tax_assets") == pytest.approx([200_000.0, 180_000.0, 160_000.0, 140_000.0])
    assert row(results, "total_assets") == pytest.approx([200_000.0, 180_000.0, 160_000.0, 140_000.0])


def test_percent_of_portfolio_retirement_withdrawal_real_mode_deflates_correctly():
    husband = make_person(age=70, retire_age=65)
    husband_portfolio = make_portfolio(equity_post=200_000.0)
    infl = 0.03
    ...
    for key in ["post_tax_assets", "total_assets"]:
        assert_discounted(raw_results, real_results, key, infl)

    assert_discounted_class(raw_results, real_results, "withdrawal", infl)