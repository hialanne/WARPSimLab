...
def make_portfolio(*, equity_pre=0.0):
    return SimpleNamespace(
...
def test_rmd_only_scenario_nominal():
    husband = make_person(age=72, retire_age=65)
    husband_portfolio = make_portfolio(equity_pre=270_000.0)
...
def test_rmd_only_scenario_real_mode_deflates_correctly():
    husband = make_person(age=72, retire_age=65)
    husband_portfolio = make_portfolio(equity_pre=270_000.0)
    infl = 0.03
    ...
    for key in ["gross_income", "pre_tax_assets"]:
