...
def test_fixed_dollar_retirement_withdrawal_nominal():
    ...
    assert row_class(results, "withdrawal") == pytest.approx([0.0, 10_000.0, 10_000.0, 10_000.0])
    assert row(results, "post_tax_assets") == pytest.approx([100_000.0, 90_000.0, 80_000.0, 70_000.0])
    assert row(results, "total_assets") == pytest.approx([100_000.0, 90_000.0, 80_000.0, 70_000.0])


def test_fixed_dollar_retirement_withdrawal_real_mode_deflates_correctly():
    husband = make_person(age=70, retire_age=65)
    husband_portfolio = make_portfolio(equity_post=100_000.0)
    infl = 0.03
